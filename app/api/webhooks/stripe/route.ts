...
async function notifyClient(session: Stripe.Checkout.Session) {
  ...
  const platformFee = Math.round(amountTotal * 0.0565) + 20;
  ...
  const html = `
    ...
    <p style="color:#aaa;font-size:11px;margin-top:6px">Fees include Stripe processing and platform fee (5.65% + 20p).</p>
