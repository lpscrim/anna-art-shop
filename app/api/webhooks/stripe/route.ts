...
async function notifyClient(session: Stripe.Checkout.Session) {
  ...
  const platformFee = Math.round(amountTotal * 0.035) + 20;
  ...
  const html = `
    ...
    <p style="color:#aaa;font-size:11px;margin-top:6px">Fees include Stripe processing (up to 3.25% + 20p for international cards) and platform fee (1%).</p>
